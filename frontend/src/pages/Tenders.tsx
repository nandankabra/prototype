...
export function TenderList({ user }: Props) {
  ...
  return <div className="page">
    ...
    <div className="toolbar"><label className="search"><Search size={19} /><input aria-label="Search tenders" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by tender title or GeM bid number" /></label><span className="toolbar-count">{tenders.length} {tenders.length === 1 ? 'tender' : 'tenders'}</span></div>
    {loading && <Loading />}{error && <ErrorBox message={error} />}
    {!loading && !error && !tenders.length && (search ? <Empty title="No matching tenders">Try another title or GeM bid number.<br/><button className="button small" onClick={()=>setSearch('')}>Clear search</button></Empty> : <Empty title="No GeM tenders imported yet">{canImport ? <><span>Import the official GeM PDF to create a verified tender record.</span><br/><button className="btn primary" onClick={() => setImportOpen(true)}><Upload size={17} /> Import official GeM document</button></> : 'A procurement officer can import an official GeM tender document.'}</Empty>)}
